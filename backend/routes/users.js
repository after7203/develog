...
router.get("/series", async (req, res) => {
  try {
    const user = await User.findOne({ id: req.query.id }).populate({
      path: "series",
      populate: { path: "boards", populate: "writer" },
    });
...
router.post("/series", auth, async (req, res) => {
  try {
    ...
    if (req.decoded.id !== user.id)
      throw new Error("mismatch", req.decoded.id, " !== ", user.id);
  ...
  async (req, res) => {
    try {
      ...
      user = await User.updateOne(
        ...
        {
          $set: {
            profile: `${process.env.REACT_APP_SERVER_URI}/public/users/${
              req.decoded.id
            }/profile/${decodeURIComponent(req.headers.profile)}`,
          },
        }
      );
      return res.json({
...
router.delete("/:mongooseId/profile", auth, async (req, res) => {
  try {
    ...
    await User.updateOne(
      ...
      {
        $set: {
          profile: `https://velog.velcdn.com/images/after7203/profile/2d5b9fac-b879-4451-97a3-54e486942c48/social_profile.png`,
        },
      }
    );
...
router.put("/:mongooseId/description", auth, async (req, res) => {
  try {
    if (req.decoded.id !== req.body.id)
      throw new Error("mismatch", req.decoded.id, " !== ", req.body.id);
    await User.updateOne(
      { _id: req.params.mongooseId },
