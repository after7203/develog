...
const Reply = ({
  ...
  const ResizeReviseform = () => {
    ...
    revise_textarea.current.style.height =
      revise_textarea.current.scrollHeight - 20 + "px";
  };
  const ResizeAddform = () => {
    add_textarea.current.style.height = "auto";
    add_textarea.current.style.height =
      add_textarea.current.scrollHeight - 20 + "px";
  };
  const invalid_toast = () =>
    toast.error("로그인후 이용해주세요", { theme: "colored", autoClose: 1500 });
  const onDelete = () => {
    axios
      .delete(`${process.env.REACT_APP_SERVER_URI}/api/reply/${reply._id}`)
      .then(() => {
        getBoard();
      });
  };
  ...
  const onSubmitReply = () => {
    ...
    axios
      .post(
        `${process.env.REACT_APP_SERVER_URI}/api/reply/${board._id}/${reply._id}`,
        {
          parentReply: reply._id,
          userId: user.id,
          mongooseId: user._id,
          contents: add_textarea.current.value,
        }
      )
      .then(() => {
        setIsLoading(false);
  ...
  const onRevise = () => {
    axios
      .put(`${process.env.REACT_APP_SERVER_URI}/api/reply/${reply._id}`, {
        writer: reply.writer.id,
        contents: revise_textarea.current.value,
      })
      .then(() => {
        getBoard();
        setIsRevise(false);
  ...
  return (
    <div className="reply">
      ...
      <div className="header">
        <div className="left">
          <img
            src={reply.writer.profile}
            alt=""
            onClick={() => navigate(`/@${reply.writer.id}`)}
          />
          <div>
            <h3 onClick={() => navigate(`/@${reply.writer.id}`)}>
              {reply.writer.id}
            </h3>
      ...
      {!isRevise ? (
        ...
        <div className="contents">
          ...
          <div className="revise_wrapper">
            ...
            <div className="rpbtn" onClick={onRevise}>
              댓글 수정
            </div>
          </div>
        </div>
      ...
      <div className="expand_reply">
        {!isExpand && (
          <div
            onClick={() => {
              setIsExpand(true);
              setExpandList([...expandList, reply._id]);
            }}
          >
        ...
        <div className="re_reply_wrapper">
          ...
          {isExpandForm ? (
            <>
              <textarea
                ref={add_textarea}
                onChange={ResizeAddform}
                placeholder="댓글을 작성하세요"
              />
              <div className="rpbtn_wrapper">
                <div
                  className="cancelbtn"
                  onClick={() => setIsExpandForm(false)}
                >
                  취소
                </div>
                <div className="rpbtn" onClick={onSubmitReply}>
                  댓글 작성
                </div>
              </div>
            </>
            ...
            <div
              className="expandform_btn"
              onClick={() => setIsExpandForm(true)}
            >
              답글 작성하기
            </div>
          )}
