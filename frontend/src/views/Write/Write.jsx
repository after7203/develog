...
const Write = () => {
  ...
  const toast_null_title = () =>
    toast.error("제목을 입력해주세요", { theme: "colored", autoClose: 1500 });
  const toast_null_contents = () =>
    toast.error("내용을 입력해주세요", { theme: "colored", autoClose: 1500 });
  const toast_duplicate_name = () =>
    toast.error("이미 존재하는 이름입니다", {
      theme: "colored",
      autoClose: 1500,
    });
  const toast_duplicate_url = () =>
  ...
  useEffect(() => {
    ...
    if (location.state) {
      (async () => {
        if (
          ...
          "https://velog.velcdn.com/images/after7203/post/10dffdfa-ff34-448b-906e-a341e560d1b6/image.png"
        ) {
    ...
    document
      .getElementsByClassName("tag_form")[0]
      .addEventListener("blur", () => setIsFocus(false));
    ...
    if (user) {
      ...
      (async () => {
        const res = await axios.get(
          `${process.env.REACT_APP_SERVER_URI}/api/users/series`,
          { params: { id: user.id } }
        );
  ...
  const deleteTag = (e) => {
    setInput({
      ...input,
      tags: input.tags.filter((tag) => tag != e.target.innerText),
    });
  };
  ...
  const setPublic = () => {
    ...
    document.getElementsByClassName("private")[0].className =
      "private unselect";
  };
  ...
  const setDetailModal = () => {
    if (input.title === "") {
      ...
      document.getElementsByClassName("detail_modal")[0].className =
        "detail_modal on";
      const brief =
        isCustomBrief || location.state
          ? input.brief
          : document
              .getElementsByClassName("ProseMirror toastui-editor-contents")[0]
              .innerText.split("\n")
              .filter((e) => e !== "")[0] || "";
  ...
  const addSeries = async () => {
    ...
    const res = await axios.post(
      `${process.env.REACT_APP_SERVER_URI}/api/users/series`,
      { user: user.id, name: name, url: user_url }
    );
    setIsLoading(false);
    setUserSeries([
      ...userSeries,
      { name: name, url: user_url, _id: res.data.series._id },
    ]);
  };
  ...
  const confirmURLSelect = () => {
    const series = Array.from(
      document.getElementsByClassName("series_list")[0].children
    )
      .map((div) => {
  ...
  const onSubmit = async () => {
    ...
    const added_series = after_series.filter(
      (_id) => !before_series.includes(_id)
    );
    const deleted_series = before_series.filter(
      (_id) => !after_series.includes(_id)
    );
    let data = {
      ...input,
      writer: user.id,
    ...
    formdata.append(
      "files",
      new File([ref_editor.current.getInstance().getHTML()], "contents.txt", {
        type: "text/plain",
      })
    );
    if (thumbPreview) {
    ...
    const headers = {
      headers: { data: encodeURIComponent(JSON.stringify(data)) },
    };
    setIsLoading(true);
    if (!location.state) {
      axios
        .post(
          `${process.env.REACT_APP_SERVER_URI}/api/board/${user.id}/${input.url}`,
          formdata,
          headers
        )
      ...
      axios
        .put(
          `${process.env.REACT_APP_SERVER_URI}/api/board/${location.state._id}`,
          formdata,
          headers
        )
        .catch((e) => console.log(e))
      ...
      <div className="form_wrap">
        <form className="write_form">
          ...
          <div className="tag_wrapper">
            {input.tags.map((tag) => (
              <div className="tag" key={tag} onClick={deleteTag}>
                {tag}
              </div>
            ))}
            <input
          ...
          <div className={isFocus ? "tip_wrapper focus" : "tip_wrapper"}>
            <div className="tip">
              쉼표 혹은 엔터를 입력하여 태그를 등록 할 수 있습니다. 등록된
              태그를 클릭하면 삭제됩니다.
            </div>
          </div>
          ...
          <div className="below_bar">
            ...
            <div>
              ...
              <div className="post" onClick={setDetailModal}>
                {!location.state ? "출간하기" : "수정하기"}
              </div>
            </div>
          </div>
        </form>
          ...
          <div className="left">
            ...
            {thumbPreview && (
              <div className="thumb_edit_btn_wrapper">
                <h3
                  onClick={() => {
                    document.getElementById("thumb_btn").click();
                  }}
                >
                  재업로드
                </h3>
                <h3
                  onClick={() => {
                    ...
                    document.getElementById("thumb_btn").value = null;
                  }}
                >
                  제거
                </h3>
            ...
            />
            <div className="count">
              <div>{input.brief.length}/150</div>
            </div>
          </div>
          <div className="right">
            {!handleSeries && (
              <>
                ...
                <div className="btn_wrapper_1">
                  <div
                    className={`public ${
                      input.scope === "public" ? "select" : "unselect"
                    }`}
                    onClick={setPublic}
                  >
                  ...
                  <div
                    className={`private ${
                      input.scope === "private" ? "select" : "unselect"
                    }`}
                    onClick={setPrivate}
                  >
                    <svg width="24" height="24" fill="none" viewBox="0 0 24 24">
                ...
                <div className="below_series_label">
                  <div
                    className="btn_series_expand"
                    onClick={() => setHandleSeries(true)}
                  >
                  ...
                  {input.series.length >= 1 && (
                    <>
                      <div className="edit_series">
                        <div onClick={() => setHandleSeries(true)}>
                          수정하기
                        </div>
                      </div>
                      <div className="loaded_series">
                        {input.series.map((series) => (
                          <div key={series._id}>{series.name}</div>
                        ))}
                      </div>
                    </>
                  )}
                ...
                <div className="btn_wrapper_2">
                  <div className="cancel" onClick={closeDetailModal}>
                    뒤로
                  </div>
                  <div className="upload" onClick={onSubmit}>
                    {!location.state ? "출간하기" : "수정하기"}
                  </div>
                </div>
              </>
            ...
            {handleSeries && (
              <div className="series_form">
                ...
                <div className="series_add" ref={ref_series_add}>
                  <input
                    className="series_add_form"
                    onKeyPress={(e) => {
                      e.key === "Enter" && addSeries();
                    }}
                    placeholder="새로운 시리즈 이름을 입력하세요"
                    onFocus={expandSeriesAddForm}
                    onChange={(e) =>
                      !isCustomSeriesURL &&
                      (series_url_preview.current.value = e.target.value)
                    }
                  />
                  <div className="url_preview">
                    ...
                    <input
                      ...
                      onChange={() => setIsCustomSeriesURL(true)}
                      ref={series_url_preview}
                    />
                  </div>
                  <div className="add_series_btn_wrapper">
                    <div
                      className="cancel_add_series_btn"
                      onClick={shrinkSeriesAddForm}
                    >
                      취소
                    </div>
                    <div className="add_series_btn" onClick={addSeries}>
                      시리즈 추가
                    </div>
                  </div>
                </div>
                <div className="series_list">
                  {userSeries.map((userSeries) => (
                    <div
                      id={JSON.stringify(userSeries)}
                      key={userSeries._id}
                      className={
                        Array.from(
                          input.series.map((series) => series.name)
                        ).includes(userSeries.name)
                          ? "select"
                          : ""
                      }
                      onClick={toggleSeriesSelect}
                    >
                      {userSeries.name}
                    </div>
                  ))}
