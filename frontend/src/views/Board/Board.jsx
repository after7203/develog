...
const Board = () => {
  ...
  const getBoard = () => {
    ...
    axios
      .get(
        `${process.env.REACT_APP_SERVER_URI}/api/board/${writer}/${boardURL}`
      )
      .then((res) => {
        setIsLoading(false);
        if (res.data.private) {
  ...
  const onSubmitReply = () => {
    ...
    setIsLoading(true);
    axios
      .post(`${process.env.REACT_APP_SERVER_URI}/api/reply/${board._id}`, {
        parentBoard: board._id,
        userId: user.id,
        mongooseId: user._id,
        contents: textarea.current.value,
      })
      .then(() => {
  ...
  const handleLike = () => {
    ...
    if (!recommend.current.classList.contains("check")) {
      axios.put(
        `${process.env.REACT_APP_SERVER_URI}/api/board/${board._id}/like`,
        { user: user.id, mongooseId: user._id }
      );
      // const new_like =
      ...
      axios.put(
        `${process.env.REACT_APP_SERVER_URI}/api/board/${board._id}/unlike`,
        { user: user.id, mongooseId: user._id }
      );
      // const new_like =
      // console.log(board.like, new_like)
      setBoard({
        ...board,
        like: board.like.filter((_id) => _id !== user._id),
      });
        ...
        <>
          <div className="board">
            ...
            <div className="title_below">
              <div>
                <h4 onClick={() => navigate(`/@${board.writer.id}`)}>
                  {board.writer.id}
                </h4>
            ...
            <div className="profile">
              <img
                src={board.writer.profile}
                onClick={() => navigate(`/@${board.writer.id}`)}
              />
              <div className="profile_text">
                <h2 onClick={() => navigate(`/@${board.writer.id}`)}>
                  {board.writer.id}
                </h2>
            ...
            <div className="reply_count">
              {getReplyCount(board.reply.length)}개의 댓글
            </div>
            <textarea
              ref={textarea}
            ...
            {board.reply.map((reply) => (
              <Reply
                ...
                reply={reply}
                _isExpand={expandList.includes(reply._id)}
