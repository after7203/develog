...
const Board = () => {
    ...
    const mongooseId = localStorage.getItem("mongoose_id") ? localStorage.getItem("mongoose_id") : sessionStorage.getItem("mongoose_id")
    useEffect(() => {
        getBoard()
    ...
    const onSubmitReply = () => {
        axios.post(`${process.env.REACT_APP_SERVER_URI}/api/reply/${writer}/${boardURL}`, { parentBoard: board._id, userId: userId, mongooseId: mongooseId, contents: textarea.current.value }).then(() => {
        ...
        <>
            {board &&
                <div className="board">
                    ...
                    <div className='reply_count'>{board.reply.length}개의 댓글</div>
                    ...
                    {board.reply.map(reply => (<Reply key={JSON.stringify(reply)} reply={reply} board={board} getBoard={getBoard} />))}
                </div>
            }
        </>
    )
